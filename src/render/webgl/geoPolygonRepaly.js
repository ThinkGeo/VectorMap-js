...
import {linearRingIsClockwise}  from 'ol/geom/flat/orient.js';
...
export class GeoPolygonReplay extends PolygonReplay {
  ...
  drawPolygon (polygonGeometry, feature) {
    ...
    if (ends.length > 0) {            
        ...
        if(ends[0] > 6) {
            outerRing = translate(flatCoordinates, 0, ends[0],
        ...
        for (i = 1, ii = ends.length; i < ii; ++i) {
            if (ends[i] !== ends[i - 1] && (ends[i] - ends[i - 1] > 6)) {
                ...
                if(holeFlatCoords.length > 6){
                    isClockwise =linearRingIsClockwise(holeFlatCoords, 0, holeFlatCoords.length, stride);
                    if(isClockwise){
                        if(!outers[++index]){
                            outers[index] = [];
                        }
